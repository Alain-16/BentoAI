...
def build_run_graph(
    orchestrator: "ShoppingOrchestrator", max_steps: int, checkpointer=None
):
    ...
    # With a checkpointer every node's state is written down as it runs, so a
    # mission's graph history can be read back afterwards. Without one the
    # graph still works - it just forgets.
...
class ShoppingOrchestrator:
    ...
    def steps(self) -> dict[MissionStatus, WorkflowStep]:
        ...
        Exposed because the graph builds its nodes from this - graph.py stays
